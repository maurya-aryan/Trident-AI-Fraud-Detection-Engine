...
// Support embedded context (Streamlit) with base64 image or fallback to public path
const BALL_IMAGE_URL = (typeof window !== 'undefined' && window.GMAIL_BALL_IMAGE) || "/gmail-ball.png";
...
export default function EmailFunnel() {
  ...
  // Handle clicks on FRAUD/SAFE endpoints for navigation
  useEffect(() => {
    const canvas = canvasRef.current;
    if (!canvas) return;
    ...
    const handleCanvasClick = (event) => {
      ...
      // Check if click is on FRAUD endpoint (left circle + box area)
      const fraudCircleDistance = Math.sqrt((clickX - LEX) ** 2 + (clickY - ENY) ** 2);
      const fraudBoxArea = (
        clickX >= LEX - ENDPOINT_BOX_W / 2 &&
        clickX <= LEX + ENDPOINT_BOX_W / 2 &&
        clickY >= ENY + ENDPOINT_BOX_OFFSET_Y &&
        clickY <= ENY + ENDPOINT_BOX_OFFSET_Y + ENDPOINT_BOX_H
      );

      if (fraudCircleDistance <= 40 || fraudBoxArea) {
        navigate('/alerts/fraud');
        return;
      }

      // Check if click is on SAFE endpoint (right circle + box area)
      const safeCircleDistance = Math.sqrt((clickX - REX) ** 2 + (clickY - ENY) ** 2);
      const safeBoxArea = (
        clickX >= REX - ENDPOINT_BOX_W / 2 &&
        clickX <= REX + ENDPOINT_BOX_W / 2 &&
        clickY >= ENY + ENDPOINT_BOX_OFFSET_Y &&
        clickY <= ENY + ENDPOINT_BOX_OFFSET_Y + ENDPOINT_BOX_H
      );

      if (safeCircleDistance <= 40 || safeBoxArea) {
        navigate('/alerts/safe');
      }
    };
    ...
    // Make the canvas cursor pointer when hovering over endpoints
    const handleMouseMove = (event) => {
      ...
      const fraudCircleDistance = Math.sqrt((mouseX - LEX) ** 2 + (mouseY - ENY) ** 2);
      const fraudBoxArea = (
        mouseX >= LEX - ENDPOINT_BOX_W / 2 &&
        mouseX <= LEX + ENDPOINT_BOX_W / 2 &&
        mouseY >= ENY + ENDPOINT_BOX_OFFSET_Y &&
        mouseY <= ENY + ENDPOINT_BOX_OFFSET_Y + ENDPOINT_BOX_H
      );

      const safeCircleDistance = Math.sqrt((mouseX - REX) ** 2 + (mouseY - ENY) ** 2);
      const safeBoxArea = (
        mouseX >= REX - ENDPOINT_BOX_W / 2 &&
        mouseX <= REX + ENDPOINT_BOX_W / 2 &&
        mouseY >= ENY + ENDPOINT_BOX_OFFSET_Y &&
        mouseY <= ENY + ENDPOINT_BOX_OFFSET_Y + ENDPOINT_BOX_H
      );

      if (fraudCircleDistance <= 40 || fraudBoxArea || safeCircleDistance <= 40 || safeBoxArea) {
